...
![Knowledge Core home screen](images/01-home.jpg)
...
1. Open http://localhost:5173. The sidebar lists every document already indexed in the shared
   knowledge base, with an input at the bottom for asking questions.

2. Upload one or more documents (PDF, TXT, MD) via the attach icon in the input bar or by
   dragging files in. Each file is parsed, chunked, embedded, and stored in the shared knowledge
   base, then shows up under **Documents** in the sidebar.

3. Ask a question in plain language.

   ![Typing a question into the chat input](images/02-ask-question.jpg)

4. The answer is generated only from the uploaded documents and includes numbered citations
   after each claim.

   ![Answer with numbered source citations](images/03-answer-citations.jpg)

5. Click a citation number to expand the exact passage it came from, with the source file and
   page number.

   ![Expanded citation showing the source passage](images/04-citation-expanded.jpg)

6. Ask follow-up questions; the assistant uses the conversation so far to stay on topic (for
   example, asking "can a customer get a complete refund?" right after a question about the
   return policy).

7. If an answer genuinely isn't in the documents, the assistant says so instead of guessing.

   ![Assistant reporting no answer found in documents](images/05-not-found.jpg)

8. To delete a document, click **Admin** in the sidebar and enter the admin key. Deletion is
   restricted to admins; anyone without the key can upload but not remove files.
